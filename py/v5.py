...
def generate_report(convict_name, attachment_img):
    if ((convict_name == '') and (attachment_img == '')):
        cdt = get_current_date_time()
        lm = get_last_mission()
        convict_info = 'No convict detected during this flight'
        ...
        convict_name = 'No convict detected during this flight'
        convict_age = 'No convict detected during this flight'
        prev_case = 'No convict detected during this flight'
        registered_face = ''
...
def on_press(key):
    ...
    try:
        ...
        if(key.char == 'l') or (key.char == 'L'):
            print(f'{len(rec_names)} faces recognized')
            me.land()
            me.streamoff()
            global can_update_telemetry
            can_update_telemetry = True
            generate_report('', '')

            pass
        if(key.char == 't') or (key.char == 'T'):
            can_update_telemetry = False
            me.takeoff()
            pass
...
def on_release(key):
    ...
    if key == keyboard.Key.esc:
        # Stop listener
        return False
def telemetry_thread_fn(name):
    print('Starting telemetry thread...')
    telemetry_data = {}
    while True:
        if(can_update_telemetry):
...
def sos_thread(name):
    while 1:
        with open('sos.pickle', 'rb') as handle:
            sos = pickle.load(handle)['sos']
        if sos:
            print('Send SOS')
            attachment_img = f'imgs/sos/sos.jpg'
            img_url = upload_rec_face(attachment_img)
            msg = 'msg here'
            ts = int(time.time())
            params = f'?img_url={img_url}&msg={msg}&ts={ts}'
            url = 'https://node-red-arzjk-2021-01-25.eu-gb.mybluemix.net/add-notification-db'+params
            res = requests.get(url).json()
            print(res)
...
def frame_stream_thread(name):
    ...
    while True:
        ...
        sos_img_fn = f'imgs/sos/sos.jpg'
        cv2.imwrite(sos_img_fn, frame, [int(cv2.IMWRITE_JPEG_QUALITY), 90])
...
# Start SOS Thread
sos_thread = threading.Thread(target=sos_thread, args=(1,))
sos_thread.start()
...
telemetry_thread = threading.Thread(target=telemetry_thread_fn, args=(1,))
